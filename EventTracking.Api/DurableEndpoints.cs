...
public static class DurableEndpoints
{
    ...
    {
        app.MapPost("/v1/events/batch", async (BatchEventRequest request, HttpContext context, PostgresStore store,
            EventValidation validation, StorageOptions storage, CancellationToken ct) =>
        {
            Dictionary<string, string[]> errors = [];
            if (request.Events is not { Length: > 0 and <= 100 })
                return Results.ValidationProblem(new Dictionary<string, string[]> { ["events"] = ["Provide 1–100 events."] });
            var now = DateTimeOffset.UtcNow;
            for (int i = 0; i < request.Events.Length; i++)
            {
                if (request.Events[i] is not { } item) { errors[$"events[{i}]"] = ["Event cannot be null."]; continue; }
                foreach (var error in validation.Validate(item, now)) errors[$"events[{i}].{error.Key}"] = error.Value;
            }
        ...
        }).WithName("TrackEventBatch").WithMetadata(new ProjectPermission("ingest"), new BodyLimit(1024 * 1024)).RequireRateLimiting("ingestion")
            .Produces<BatchAcceptance>(200).Produces<BatchAcceptance>(202).ProducesValidationProblem().ProducesProblem(409)
            .ProducesProblem(401).ProducesProblem(403).ProducesProblem(413).ProducesProblem(429).ProducesProblem(503)
            .WithDescription("Atomically accept 1–100 events. 1 MiB body, 32 KiB per event. Any validation error or ID conflict rolls back the whole batch.");
    ...
    {
        var errors = PostgresAnalytics.Validate(filter);
        return errors.Count > 0 ? Results.ValidationProblem(errors) : Results.Ok(await analytics.SummaryAsync(context.Project().ProjectId, filter, ct));
    }
}
