...
public sealed record RevenueTotal(
    string Currency,
    long GrossMinor,
    long RefundMinor,
    long NetMinor,
...
{
    public async Task<IReadOnlyList<FunnelResult>> FunnelAsync(
        string project,
        DateTimeOffset from,
        DateTimeOffset to,
        TimeSpan window,
    ...
    )
    {
        ...
        await using var q = DatabaseSql.Command(
            c,
            null,
            "SELECT user_id,event_type,occurred_at,event_id FROM events WHERE project_id=$1 AND user_id IS NOT NULL AND occurred_at >= $2 AND occurred_at < $3 ORDER BY user_id,occurred_at,event_id",
            project,
            from,
            to
        );
        ...
        while (await reader.ReadAsync(ct))
        {
            ...
            if (!byUser.TryGetValue(user, out var list))
                byUser[user] = list = [];
            list.Add((reader.GetString(1), reader.GetFieldValue<DateTimeOffset>(2)));
        }
        ...
        foreach (var events in byUser.Values)
        {
            ...
            for (int step = 0; step < steps.Count; step++)
            {
                var match = events.FirstOrDefault(e =>
                    e.Type == steps[step].EventType
                    && (previous is null || (e.At >= previous && e.At <= previous + window))
                );
                if (match == default)
                    break;
                counts[step]++;
        ...
        return steps
            .Select((step, i) => new FunnelResult(i + 1, step.EventType, counts[i]))
            .ToArray();
    }

    public async Task<IReadOnlyList<RetentionResult>> RetentionAsync(
        string project,
        DateOnly start,
        DateOnly end,
        string entry,
    ...
    )
    {
        ...
        var rows = new List<RetentionResult>();
        await using var r = await q.ExecuteReaderAsync(ct);
        while (await r.ReadAsync(ct))
        {
            var date = r.GetFieldValue<DateOnly>(0);
            int day = r.GetInt32(1);
            rows.Add(
                new(
                    date,
                    day,
                    r.GetInt64(2),
                    r.GetInt64(3),
                    date.AddDays(day + 1) <= DateOnly.FromDateTime(now.UtcDateTime)
                )
            );
        }
        return rows;
    ...
    public async Task<IReadOnlyList<SessionSummary>> SessionsAsync(
        string project,
        DateTimeOffset from,
        DateTimeOffset to,
        CancellationToken ct
    )
    {
        ...
        await using var q = DatabaseSql.Command(
            c,
            null,
            "SELECT session_id,min(user_id),min(occurred_at),max(occurred_at),count(*) FROM events WHERE project_id=$1 AND session_id IS NOT NULL AND occurred_at >= $2 AND occurred_at < $3 GROUP BY session_id ORDER BY min(occurred_at),session_id",
            project,
            from,
            to
        );
    ...
    public async Task<IReadOnlyList<RevenueTotal>> RevenueAsync(
        string project,
        DateTimeOffset from,
        DateTimeOffset to,
        CancellationToken ct
