...
public sealed class DashboardAccounts(NpgsqlDataSource source)
{
    ...
    private static readonly string DummyHash =
        new PasswordHasher<DashboardUserRecord>().HashPassword(Dummy, Guid.NewGuid().ToString());

    public async Task<DashboardUserRecord?> Authenticate(
        string? username,
        string? password,
        CancellationToken ct
    )
    {
        if (username is null || username.Length > 60 || password is null || password.Length > 256)
            return null;
        await using var connection = await source.OpenConnectionAsync(ct);
        await using var command = DatabaseSql.Command(
            connection,
            null,
            "SELECT id,username,password_hash,disabled,session_version FROM dashboard_users WHERE username=$1",
        ...
        await using (var reader = await command.ExecuteReaderAsync(ct))
            if (await reader.ReadAsync(ct))
                user = new()
                {
                    ...
                    SessionVersion = reader.GetInt32(4),
                };
        ...
        return user is { Disabled: false } && result != PasswordVerificationResult.Failed
            ? user
            : null;
    }
    ...
    public async Task<IReadOnlyList<DashboardProject>> Projects(Guid userId, CancellationToken ct)
    {
        ...
        await using var command = DatabaseSql.Command(
            connection,
            null,
            "SELECT p.id,coalesce(nullif(p.name,''),p.id),m.can_demo,m.can_manage FROM projects p JOIN project_memberships m ON m.project_id=p.id WHERE m.user_id=$1 ORDER BY p.id",
            userId
        );
        await using var reader = await command.ExecuteReaderAsync(ct);
    ...
    public async Task<DashboardProject?> CreateProject(
        Guid userId,
        string name,
        CancellationToken ct
    )
    {
        ...
        await using (
            var user = DatabaseSql.Command(
                connection,
                transaction,
                "SELECT id FROM dashboard_users WHERE id=$1 AND NOT disabled FOR UPDATE",
                userId
            )
        )
            if (await user.ExecuteScalarAsync(ct) is null)
                return null;
        await using (
            var count = DatabaseSql.Command(
                connection,
                transaction,
                "SELECT count(*) FROM project_memberships WHERE user_id=$1",
                userId
            )
        )
        ...
        await using (
            var project = DatabaseSql.Command(
                connection,
                transaction,
                "INSERT INTO projects(id,name,event_count,stored_bytes) VALUES($1,$2,0,0)",
                id,
                name
            )
        )
        ...
        await using (
            var member = DatabaseSql.Command(
                connection,
                transaction,
                "INSERT INTO project_memberships(user_id,project_id,can_demo,can_manage) VALUES($1,$2,true,true)",
                userId,
                id
            )
        )
    ...
    public async Task<bool> Provision(IConfiguration config, CancellationToken ct = default)
    {
        ...
        string[] projects = (config["Dashboard:Bootstrap:Projects"] ?? "a").Split(
            ',',
            StringSplitOptions.TrimEntries | StringSplitOptions.RemoveEmptyEntries
        );
        if (
            username.Length is < 3 or > 60
            || username.Any(c => !char.IsAsciiLetterOrDigit(c) && c is not ('-' or '_' or '.'))
            || password.Length is < 16 or > 256
            || projects.Length is < 1 or > 20
            || projects.Any(p => p.Length > 100 || p == "demo-shop" || p.Any(char.IsControl))
        )
            throw new InvalidOperationException(
                "Dashboard bootstrap requires a 3–60 character username, 16–256 character password and 1–20 comma-separated project IDs."
            );
        ...
        foreach (string projectId in projects.Distinct())
        {
            await using (
                var project = DatabaseSql.Command(
                    connection,
                    transaction,
                    "INSERT INTO projects(id,name,event_count,stored_bytes) VALUES($1,$1,0,0) ON CONFLICT(id) DO NOTHING",
                    projectId
                )
            )
            ...
            await using var member = DatabaseSql.Command(
                connection,
                transaction,
                "INSERT INTO project_memberships(user_id,project_id,can_demo) VALUES($1,$2,true)",
                account.Id,
                projectId
            );
            await member.ExecuteNonQueryAsync(ct);
