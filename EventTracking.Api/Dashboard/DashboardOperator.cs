...
public static class DashboardOperator
{
    ...
    )
    {
        ...
        if (
            action is not ("reset-password" or "disable" or "enable" or "grant-admin")
            || username.Length is < 3 or > 60
            || string.IsNullOrWhiteSpace(actor)
            || actor.Length > 100
            || actor.Any(char.IsControl)
            ...
            || (
                action == "grant-admin"
                && (string.IsNullOrWhiteSpace(project) || project.Length > 100)
            )
        )
            throw new InvalidOperationException(
                "Set Administration:Action (reset-password, disable, enable, grant-admin), Username, Actor and Reason. Reset requires a 16–256 character Password; grant-admin requires ProjectId and an existing membership."
            );
        ...
        {
            await using var locked = DatabaseSql.Command(
                connection,
                transaction,
        ...
        if (action == "grant-admin")
        {
            if (disabled)
                throw new InvalidOperationException(
                    "Enable the account before granting administrator access."
                );
            await using var grant = DatabaseSql.Command(
                connection,
                transaction,
                "UPDATE project_memberships SET can_manage=true,can_demo=true WHERE user_id=$1 AND project_id=$2",
                id,
                project
            );
            if (await grant.ExecuteNonQueryAsync(ct) != 1)
                throw new InvalidOperationException("An existing project membership is required.");
        }
        else if (action == "reset-password")
        {
            ...
            await using var reset = DatabaseSql.Command(
                connection,
                transaction,
                "UPDATE dashboard_users SET password_hash=$1,session_version=session_version+1 WHERE id=$2",
                hash,
                id
            );
        ...
        else
        {
            await using var update = DatabaseSql.Command(
                connection,
                transaction,
                "UPDATE dashboard_users SET disabled=$1,session_version=session_version+1 WHERE id=$2",
                action == "disable",
                id
            );
        ...
        await AuditLog.Write(
            ...
            "operator:" + actor,
            project,
            action == "grant-admin" ? "membership.admin-granted" : "account." + action,
            id.ToString(),
            new { reason },
            ct
        );
