...
public static class ApiEndpoints
{
    public static void MapTrackingApi(this WebApplication app)
    {
        app.MapPost("/v1/events", Accept)
            ...
            .ProducesProblem(503)
            .WithDescription(
                "Distributed: 202 after durable inbox commit. Hosted: 200 after queryable commit. Stable IDs deduplicate retries. Explicit Volatile mode retains prototype semantics."
            );
        ...
        app.MapPost(
            ...
            .WithMetadata(new ProjectPermission("ingest"))
            .RequireRateLimiting("ingestion")
            .WithName("TrackLegacyEvent")
            .WithDescription(
                "Deprecated adapter. Requires an ingest key and returns the v1 response. Generates an event ID."
            );
        ...
        foreach (string prefix in new[] { "", "/v1" })
        {
            app.MapGet(
                    ...
                    (
                        ...
                        HttpContext context,
                        IEventStore store
                    ) => Summary(context, store, from, to, null)
                )
            ...
            app.MapGet(
                    ...
                    (
                        ...
                        HttpContext context,
                        IEventStore store
                    ) => Summary(context, store, from, to, userId)
                )
                .WithMetadata(new ProjectPermission("read"))
                .RequireRateLimiting("analytics")
                .Produces<EventSummary[]>();
    ...
    )
    {
        ...
        var properties = request.Properties is { ValueKind: JsonValueKind.Object } p
            ? p.EnumerateObject()
                .ToDictionary(item => item.Name, item => (object?)item.Value.Clone())
            : null;
        var tracked = new TrackedEvent(
            request.EventId,
        ...
        loggerFactory
            .CreateLogger("Ingestion")
            .LogInformation(
                "Accepted volatile event {EventId} for project {ProjectId}",
                tracked.Id,
                tracked.ProjectId
            );
        return Results.Accepted(
            value: new EventAcceptance(tracked.Id, "accepted", now, "volatile")
        );
    }

    private static IResult Summary(
        HttpContext context,
        IEventStore store,
        DateTimeOffset? from,
        DateTimeOffset? to,
        ...
        if (from > to)
            return Results.ValidationProblem(
                new Dictionary<string, string[]>
                {
                    ["from"] =
                    [
                        "from must be earlier than or equal to to. Windows are [from, to).",
                    ],
                }
            );
        if (userId is not null && (string.IsNullOrWhiteSpace(userId) || userId.Length > 200))
            return Results.ValidationProblem(
                new Dictionary<string, string[]> { ["userId"] = ["Use 1–200 characters."] }
            );
