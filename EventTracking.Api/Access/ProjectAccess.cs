...
{
    private readonly Dictionary<string, ProjectContext> _keys = new(StringComparer.OrdinalIgnoreCase);
    ...
    public ProjectKeys(IConfiguration configuration, IHostEnvironment environment)
    {
        ...
        foreach (var key in configuration.GetSection("ProjectAccess:Keys").Get<DevelopmentKey[]>() ?? [])
        {
            ...
            if (!key.Revoked && !_keys.TryAdd(key.KeyHash, new(key.ProjectId, key.Permissions)))
                throw new InvalidOperationException("A key hash must identify exactly one project.");
        }
    }

    public static void Validate(DevelopmentKey key)
    {
        if (string.IsNullOrWhiteSpace(key.ProjectId) || key.ProjectId.Length > 100 || key.ProjectId == "demo-shop"
            || key.KeyHash is null || key.KeyHash.Length != 64 || !key.KeyHash.All(Uri.IsHexDigit)
            || key.Permissions is null || key.Permissions.Length == 0 || key.Permissions.Any(p => p is not ("ingest" or "read")))
    ...
    {
        if (token.Length is < 32 or > 256) return ValueTask.FromResult<ProjectContext?>(null);
...
public static class ProjectAccess
{
    ...
    public static async Task Authorize(HttpContext context, RequestDelegate next)
    {
        ...
        if (permission is null) { await next(context); return; }
        string authorization = context.Request.Headers.Authorization.ToString();
        var project = authorization.StartsWith("Bearer ", StringComparison.OrdinalIgnoreCase)
            ? await context.RequestServices.GetRequiredService<IProjectKeys>().AuthenticateAsync(authorization[7..], context.RequestAborted) : null;
        if (project is null)
        {
            context.Response.Headers.WWWAuthenticate = "Bearer";
            await Results.Problem(statusCode: 401, title: "A valid project API key is required.").ExecuteAsync(context);
            return;
        }
        if (!project.Permissions.Contains(permission.Name))
        {
            await Results.Problem(statusCode: 403, title: $"The key requires {permission.Name} permission.").ExecuteAsync(context);
            return;
        }
