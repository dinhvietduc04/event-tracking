...
public static class JsonNumbers
{
    ...
    public static bool IsExactDecimal(JsonElement value) => value.TryGetDecimal(out var number)
    ...
    private static string? Normalize(string text)
    {
        ...
        int exponent = 0;
        if (parts.Length == 2 && !int.TryParse(parts[1], NumberStyles.AllowLeadingSign, CultureInfo.InvariantCulture, out exponent)) return null;
        ...
        return (negative ? "-" : "") + digits[..(digits.Length - trailing)] + "e" + (scale + trailing).ToString(CultureInfo.InvariantCulture);
    }
}
