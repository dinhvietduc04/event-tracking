...
{
    public static readonly IReadOnlyList<DemoProduct> Products = Array.AsReadOnly(
        new[]
        {
    ...
    private readonly Dictionary<
        (string Session, Guid Id),
        (string Fingerprint, DemoOrder Order)
    > _orders = [];
    ...
    )
    {
        if (
            ...
            || request.Items is not { Length: > 0 and <= 3 }
            || request.Items.Any(item =>
                item is null
                || item.Quantity is < 1 or > 10
                || !Products.Any(product => product.Id == item.ProductId)
            )
            || request.Items.Select(item => item.ProductId).Distinct().Count()
        ...
        )
        {
            return Results.BadRequest(
                new
                {
                    error = "Choose 1–3 different products, with 1–10 of each, and a valid order ID.",
                }
            );
        }
        ...
        try
        {
            if (_orders.TryGetValue((session, request.OrderId), out var existing))
            {
                return existing.Fingerprint == fingerprint
                    ...
                    : Results.Conflict(
                        new { error = "That order ID was already used for a different cart." }
                    );
            }

            // Prices and purchase identity come from the shop backend, never browser telemetry.
            int total = request.Items.Sum(item =>
                Products.Single(product => product.Id == item.ProductId).PriceMinor * item.Quantity
            );
            var order = new DemoOrder(
                request.OrderId,
                total,
                "USD",
                request.Items.Sum(item => item.Quantity)
            );
...
public static class DemoShopEndpoints
{
    ...
    public static void MapDemoShop(this WebApplication app)
    {
        ...
        group.MapPost(
            "/events",
            async (
                DemoBrowserEventRequest request,
                HttpContext context,
                IEventQueue queue,
                CancellationToken cancellationToken
            ) =>
            {
                ...
                if (
                    request.EventType is not ("page_view" or "product_added" or "checkout_started")
                    || (
                        request.EventType == "product_added"
                        && !DemoShop.Products.Any(item => item.Id == request.ProductId)
                    )
                )
                {
                    return Results.BadRequest(
                        new { error = "Unsupported browser event or product." }
                    );
                }
                var now = DateTimeOffset.UtcNow;
                await queue.QueueAsync(
                    new TrackedEvent(
                        ...
                        "browser",
                        session,
                        new Dictionary<string, object?> { ["productId"] = request.ProductId }
                    ),
                    cancellationToken
                );
        ...
        group.MapGet(
            "/events",
            (HttpContext context, IEventStore store) =>
            {
                context.Response.Headers.CacheControl = "no-store";
                string? session = GetSession(context);
                return session is null
                    ? Results.Unauthorized()
                    : Results.Ok(store.GetRecent(session));
            }
        );
        ...
        group.MapPost(
            ...
            ) =>
            {
                ...
                return session is null
                    ? Results.Unauthorized()
                    : await shop.PlaceOrder(session, request, queue, cancellationToken);
            }
        );
    }
