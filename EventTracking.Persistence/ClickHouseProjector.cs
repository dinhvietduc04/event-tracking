    ...
    IHttpClientFactory clients, ILogger<ClickHouseProjector> logger)
{
    ...
    public async Task<int> ProcessBatchAsync(CancellationToken ct)
    {
        ...
        await using (var reader = await command.ExecuteReaderAsync(ct))
            while (await reader.ReadAsync(ct))
            {
                ...
                static string Stamp(DateTimeOffset value) => value.UtcDateTime.ToString("yyyy-MM-dd HH:mm:ss.ffffff");
                keys.Add((project, id));
                rows.Add(new { project_id = project, event_id = id, event_type = reader.GetString(2), schema_version = reader.GetInt32(3),
                    ...
                    session_id = reader.IsDBNull(6) ? null : reader.GetString(6), occurred_at = Stamp(reader.GetFieldValue<DateTimeOffset>(7)), received_at = Stamp(received),
                    properties = reader.GetString(9), version = Stamp(received) });
            }
    ...
    public sealed record ClickHouseReconciliation(string ProjectId, long PostgresCount, long ClickHouseCount, bool Matches);

    public async Task<ClickHouseReconciliation> ReconcileAsync(string projectId, CancellationToken ct)
    {
        if (!options.Enabled) throw new InvalidOperationException("ClickHouse is not enabled.");
        await using var connection = await source.OpenConnectionAsync(ct);
        await using var command = DatabaseSql.Command(connection, null, "SELECT count(*) FROM events WHERE project_id=$1", projectId);
        long pgCount = Convert.ToInt64(await command.ExecuteScalarAsync(ct));

        string chResult = await QueryAsync($"SELECT count(*) FROM {options.Database}.events FINAL WHERE project_id='{projectId.Replace("'", "''")}'", ct);
    ...
    public async Task<int> BackfillRangeAsync(DateTimeOffset from, DateTimeOffset to, CancellationToken ct)
    {
        if (!options.Enabled) return 0;
        await using var connection = await source.OpenConnectionAsync(ct);
        await using var transaction = await connection.BeginTransactionAsync(ct);
        ...
        await transaction.CommitAsync(ct);
        logger.LogInformation("Enqueued {Count} events for ClickHouse backfill between {From} and {To}", enqueued, from, to);
        return enqueued;
    }
