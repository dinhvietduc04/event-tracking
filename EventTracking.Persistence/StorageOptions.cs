...
public sealed class StorageOptions
{
    ...
    public void Validate(IConfiguration configuration)
    {
        ...
        if (
            Profile is not ("Volatile" or "Distributed" or "Hosted")
            || WorkerBatchSize is < 1 or > 1000
            || PollIntervalMilliseconds is < 50 or > 60000
            || RetentionDays is < 1 or > 3650
            || RetentionDays <= maxLate
            || IdentityRetentionDays < RetentionDays
            || IdentityRetentionDays <= maxLate
            || IdentityRetentionDays > 3650
            || MaxProjectEvents < 1
            || MaxProjectBytes < 1
            || MaxDatabaseBytes < 1
            || MaxOutboxPending < 1
            ...
            throw new InvalidOperationException(
                "Invalid storage profile, limits, or retention (event retention must exceed allowed lateness; identity retention must cover event retention)."
            );
    }
}
...
public sealed class ClickHouseOptions
{
    ...
    public void Validate()
    {
        ...
        if (
            ...
            || Database.Any(c => !char.IsLetterOrDigit(c) && c != '_')
        )
            throw new InvalidOperationException(
                "ClickHouse requires an absolute HTTP(S) URL and a simple database name."
            );
    }
}
...
public sealed class RabbitMqOptions
{
    ...
    public void Validate()
    {
        ...
        if (
            ...
            || string.IsNullOrWhiteSpace(DeadLetterQueue)
            || MaxAttempts is < 1 or > 100
            || Prefetch is < 1 or > 1000
        )
            throw new InvalidOperationException(
                "RabbitMQ requires a valid AMQP URI, topology names, attempts and prefetch limits."
            );
    }
}
