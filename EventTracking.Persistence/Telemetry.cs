...
public static class EventTrackingTelemetry
{
    ...
    public static readonly Counter<long> EventsAccepted = Meter.CreateCounter<long>(
        "events.accepted",
        "events",
        "Total number of events accepted into the system."
    );

    public static readonly Counter<long> EventsProjected = Meter.CreateCounter<long>(
        "events.projected",
        "events",
        "Total number of events successfully projected."
    );

    public static readonly Counter<long> EventsRejected = Meter.CreateCounter<long>(
        "events.rejected",
        "events",
        "Total number of events rejected (conflict, quota, validation)."
    );
    ...
    public static readonly Histogram<double> IngestionLatencyMs = Meter.CreateHistogram<double>(
        "ingestion.latency_ms",
        "ms",
        "Latency of event ingestion acceptance in milliseconds."
    );

    public static readonly Histogram<double> ProjectionLatencyMs = Meter.CreateHistogram<double>(
        "projection.latency_ms",
        "ms",
        "Latency of projection batch processing in milliseconds."
    );

    public static readonly Histogram<int> ProjectionBatchSize = Meter.CreateHistogram<int>(
        "projection.batch_size",
        "events",
        "Number of events in projection batches."
    );
    ...
    static EventTrackingTelemetry()
    {
        Meter.CreateObservableGauge(
            "outbox.pending",
            () => Interlocked.Read(ref OutboxPendingCache),
            "events",
            "Current unpublished broker outbox depth."
        );
        Meter.CreateObservableGauge(
            "outbox.oldest_seconds",
            () => Volatile.Read(ref OutboxOldestSecondsCache),
            "s",
            "Age of the oldest pending outbox message in seconds."
        );
        Meter.CreateObservableGauge(
            "inbox.pending",
            () => Interlocked.Read(ref InboxPendingCache),
            "events",
            "Current unprojected inbox depth."
        );
    }
    ...
    public static void SetInboxGauge(long pending) =>
        Interlocked.Exchange(ref InboxPendingCache, pending);
    ...
    public static string GeneratePrometheusMetrics()
    {
        ...
        sb.AppendLine(
            "# HELP outbox_oldest_seconds Age of the oldest pending outbox message in seconds."
        );
        ...
        sb.AppendLine(
            "# HELP events_accepted_total Total number of events accepted into the system."
        );
        ...
        sb.AppendLine(
            "# HELP events_projected_total Total number of events successfully projected."
        );
        ...
        sb.AppendLine(
            "# HELP events_dead_lettered_total Total number of events moved to dead-letter storage."
        );
