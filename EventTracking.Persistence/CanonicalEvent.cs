...
)
{
    ...
    public static DateTimeOffset Microseconds(DateTimeOffset time) =>
        new(time.UtcTicks - time.UtcTicks % 10, TimeSpan.Zero);
    ...
    public static CanonicalEvent Create(V1EventRequest request)
    {
        var normalized = request with
        {
            ...
            Properties = request.Properties is { ValueKind: JsonValueKind.Object } p
                ? p
                : JsonSerializer.SerializeToElement(new { }),
        };
        using var stream = new MemoryStream();
        using (var writer = new Utf8JsonWriter(stream))
            Write(writer, JsonSerializer.SerializeToElement(normalized, JsonOptions));
        byte[] bytes = stream.ToArray();
        return new(
            normalized,
            Encoding.UTF8.GetString(bytes),
            Convert.ToHexString(SHA256.HashData(bytes)),
            bytes.Length * 2L + 1024
        );
    }
