...
public sealed record OutboxMetrics(
    long Pending,
...
public sealed record SubjectExportEvent(
    Guid EventId,
    string EventType,
    int SchemaVersion,
    string? UserId,
...
)
{
    ...
    public async Task<IReadOnlyList<EventAcceptance>> AcceptAsync(
        string projectId,
        IReadOnlyList<V1EventRequest> requests,
    ...
    )
    {
        ...
        await using (
            var state = DatabaseSql.Command(
                connection,
                transaction,
                "SELECT profile FROM storage_state WHERE id = 1 FOR UPDATE"
            )
        )
            if ((string?)await state.ExecuteScalarAsync(ct) != options.Profile)
                throw new StorageProfileException();
        long count,
            bytes;
        await using (
            var project = DatabaseSql.Command(
                connection,
                transaction,
                "SELECT event_count, stored_bytes FROM projects WHERE id = $1 FOR UPDATE",
                projectId
            )
        )
        await using (var reader = await project.ExecuteReaderAsync(ct))
        {
            if (!await reader.ReadAsync(ct))
                throw new InvalidOperationException("Authenticated project does not exist.");
        ...
        foreach (var item in events)
        {
            if (withinBatch.TryGetValue(item.Event.EventId, out var previousReceipt))
            {
                results.Add(
                    new(
                        item.Event.EventId,
                        options.Profile == "Hosted" ? "persisted" : "accepted",
                        previousReceipt,
                        "durable",
                        true
                    )
            ...
            await using (
                var identity = DatabaseSql.Command(
                    connection,
                    transaction,
                    "SELECT payload_hash, received_at FROM event_identity WHERE project_id = $1 AND event_id = $2",
                    projectId,
                    item.Event.EventId
                )
            )
            ...
            {
                addedCount++;
                addedBytes += item.StoredBytes;
                if (
                    count + addedCount > options.MaxProjectEvents
                    || bytes + addedBytes > options.MaxProjectBytes
                )
                {
        ...
        if (addedCount > 0)
        {
            ...
            await using var size = DatabaseSql.Command(
                connection,
                transaction,
                "SELECT pg_database_size(current_database())"
            );
            if (
                Convert.ToInt64(await size.ExecuteScalarAsync(ct)) + addedBytes
            ...
            if (rabbitMq.Enabled)
            {
                await using var outboxCountCmd = DatabaseSql.Command(
                    connection,
                    transaction,
            ...
            {
                await using (
                    var insert = DatabaseSql.Command(
                        connection,
                        transaction,
                ...
                {
                    await using var inbox = DatabaseSql.Command(
                        connection,
                        transaction,
                ...
                if (clickHouse.Enabled)
                    await EnqueueClickHouseProjection(
                        connection,
                        transaction,
                        projectId,
                        item.Event.EventId,
            ...
            await using var update = DatabaseSql.Command(
                connection,
                transaction,
                "UPDATE projects SET event_count = event_count + $2, stored_bytes = stored_bytes + $3 WHERE id = $1",
                projectId,
                addedCount,
                addedBytes
            );
        ...
        EventTrackingTelemetry.RecordAccepted(
            projectId,
            events.Length,
            stopwatch.Elapsed.TotalMilliseconds
        );
        logger.LogInformation(
            "Committed {NewEvents} new and {Duplicates} repeated events for {ProjectId} in {Profile}",
            addedCount,
            events.Length - addedCount,
            projectId,
            options.Profile
        );
        return results;
    ...
    internal static async Task InsertEvent(
        NpgsqlConnection connection,
        NpgsqlTransaction transaction,
        string projectId,
        V1EventRequest item,
    ...
    internal static async Task EnqueueClickHouseProjection(
        NpgsqlConnection connection,
        NpgsqlTransaction transaction,
        string projectId,
        Guid eventId,
    ...
    internal static async Task EnqueueBrokerOutbox(
        NpgsqlConnection connection,
        NpgsqlTransaction transaction,
        string projectId,
        Guid eventId,
    ...
    )
    {
        await using var command = DatabaseSql.Command(
            connection,
            transaction,
            "INSERT INTO broker_outbox (project_id,event_id,payload,created_at,next_attempt_at,attempts) VALUES ($1,$2,$3::jsonb,now(),now(),0) ON CONFLICT (project_id,event_id) DO NOTHING",
            projectId,
            eventId,
            payload
        );
    ...
    public async Task<int> ProcessBatchAsync(CancellationToken ct)
    {
        ...
        await using (
            var state = DatabaseSql.Command(
                connection,
                transaction,
                "SELECT profile FROM storage_state WHERE id=1 FOR SHARE"
            )
        )
            if ((string?)await state.ExecuteScalarAsync(ct) != "Distributed")
                throw new StorageProfileException();
        ...
        await using (
            var claim = DatabaseSql.Command(
                connection,
                transaction,
                "SELECT project_id,payload,received_at FROM inbox WHERE processed_at IS NULL ORDER BY received_at,project_id,event_id LIMIT $1 FOR UPDATE SKIP LOCKED",
        ...
        await using (var reader = await claim.ExecuteReaderAsync(ct))
            ...
            {
                var payload =
                    JsonSerializer.Deserialize<V1EventRequest>(
                        reader.GetString(1),
                        CanonicalEvent.JsonOptions
        ...
        foreach (var item in batch)
        {
            ...
            await using var complete = DatabaseSql.Command(
                connection,
                transaction,
                "UPDATE inbox SET processed_at = now() WHERE project_id = $1 AND event_id = $2",
                item.Project,
                item.Event.EventId
            );
        ...
        if (batch.Count > 0)
        {
            foreach (var grp in batch.GroupBy(b => b.Project))
                EventTrackingTelemetry.RecordProjected(
                    grp.Key,
                    grp.Count(),
                    stopwatch.Elapsed.TotalMilliseconds
                );
            logger.LogInformation(
                "Projected {Count} durable inbox events; oldest receipt {ReceivedAt}",
                batch.Count,
                batch.Min(e => e.ReceivedAt)
            );
        }
        try
        {
            await using var gaugeConnection = await source.OpenConnectionAsync(ct);
            await using var gaugeCommand = DatabaseSql.Command(
                gaugeConnection,
                null,
                "SELECT count(*) FROM inbox WHERE processed_at IS NULL"
            );
            EventTrackingTelemetry.SetInboxGauge(
                Convert.ToInt64(await gaugeCommand.ExecuteScalarAsync(ct))
            );
        }
    ...
    public async Task<int> PublishBrokerOutboxAsync(
        Func<BrokerMessage, Task> publish,
        CancellationToken ct
    )
    {
        await using var connection = await source.OpenConnectionAsync(ct);
        ...
        await using (
            var claim = DatabaseSql.Command(
                connection,
                transaction,
                "SELECT project_id,event_id,payload FROM broker_outbox WHERE published_at IS NULL AND completed_at IS NULL AND dead_lettered_at IS NULL AND next_attempt_at <= now() ORDER BY created_at,project_id,event_id LIMIT $1 FOR UPDATE SKIP LOCKED",
                options.WorkerBatchSize
            )
        )
        await using (var reader = await claim.ExecuteReaderAsync(ct))
            while (await reader.ReadAsync(ct))
                messages.Add(new(reader.GetString(0), reader.GetGuid(1), reader.GetString(2)));
        foreach (var message in messages)
        {
            await publish(message);
            await using var mark = DatabaseSql.Command(
                connection,
                transaction,
                "UPDATE broker_outbox SET published_at=now(),last_error=NULL WHERE project_id=$1 AND event_id=$2",
                message.ProjectId,
                message.EventId
            );
    ...
    public async Task CompleteBrokerDeliveryAsync(BrokerMessage message, CancellationToken ct)
    {
        var stopwatch = System.Diagnostics.Stopwatch.StartNew();
        var item =
            JsonSerializer.Deserialize<V1EventRequest>(message.Payload, CanonicalEvent.JsonOptions)
            ?? throw new InvalidOperationException("Broker payload is invalid.");
        if (item.EventId != message.EventId)
            throw new InvalidOperationException("Broker envelope event ID does not match payload.");
        await using var connection = await source.OpenConnectionAsync(ct);
        await using var transaction = await connection.BeginTransactionAsync(ct);
        await using (
            var state = DatabaseSql.Command(
            ...
            if ((string?)await state.ExecuteScalarAsync(ct) != "Distributed")
                throw new StorageProfileException();
        await using (
            var exists = DatabaseSql.Command(
                connection,
                transaction,
                "SELECT 1 FROM broker_outbox WHERE project_id=$1 AND event_id=$2 FOR UPDATE",
                message.ProjectId,
                message.EventId
            )
        )
            if (await exists.ExecuteScalarAsync(ct) is null)
                throw new InvalidOperationException(
                    "Broker delivery does not belong to this database."
                );
        await InsertEvent(
            connection,
            transaction,
            message.ProjectId,
            item,
            await ReceiptAsync(connection, transaction, message.ProjectId, message.EventId, ct),
            ct
        );
        if (clickHouse.Enabled)
            await EnqueueClickHouseProjection(
        ...
        await using (
            ...
            await inbox.ExecuteNonQueryAsync(ct);
        await using (
            var outbox = DatabaseSql.Command(
                connection,
                transaction,
                "UPDATE broker_outbox SET completed_at=coalesce(completed_at,now()),last_error=NULL WHERE project_id=$1 AND event_id=$2",
                message.ProjectId,
                message.EventId
            )
        )
    ...
    public Task RecordBrokerFailureAsync(
        BrokerMessage message,
        Exception error,
        CancellationToken ct
    ) => RecordBrokerFailureAsync(message, error, permanent: false, ct);

    public async Task RecordBrokerFailureAsync(
        BrokerMessage message,
        Exception error,
        bool permanent,
        CancellationToken ct
    )
    {
        ...
        await using var command = DatabaseSql.Command(
            connection,
            transaction,
            "SELECT attempts FROM broker_outbox WHERE project_id=$1 AND event_id=$2 FOR UPDATE",
            message.ProjectId,
            message.EventId
        );
        ...
        {
            await using var dead = DatabaseSql.Command(
                connection,
                transaction,
                "UPDATE broker_outbox SET attempts=$3,dead_lettered_at=now(),last_error=$4 WHERE project_id=$1 AND event_id=$2",
                message.ProjectId,
                message.EventId,
                attempts,
                detail
        ...
        else
        {
            ...
            await using var retry = DatabaseSql.Command(
                connection,
                transaction,
                "UPDATE broker_outbox SET attempts=$3,published_at=NULL,next_attempt_at=now()+$4::interval,last_error=$5 WHERE project_id=$1 AND event_id=$2",
                message.ProjectId,
                message.EventId,
                attempts,
                delay.ToString("c"),
    ...
    public async Task<int> ReplayDeadLettersAsync(string projectId, CancellationToken ct)
    {
        ...
        await using var command = string.IsNullOrWhiteSpace(projectId)
            ? DatabaseSql.Command(
                connection,
                transaction,
                "UPDATE broker_outbox SET dead_lettered_at=NULL,published_at=NULL,next_attempt_at=now(),attempts=0,last_error=NULL WHERE dead_lettered_at IS NOT NULL"
            )
            : DatabaseSql.Command(
                connection,
                transaction,
    ...
    public async Task<IReadOnlyList<DeadLetterRecord>> GetDeadLettersAsync(
        string projectId,
        int limit,
        CancellationToken ct
    )
    {
        ...
        await using var command = DatabaseSql.Command(
            connection,
            null,
            "SELECT event_id,project_id,created_at,attempts,last_error,dead_lettered_at FROM broker_outbox WHERE project_id=$1 AND dead_lettered_at IS NOT NULL ORDER BY dead_lettered_at DESC,event_id LIMIT $2",
            projectId,
    ...
    public static async Task<int> DeleteSubject(
        NpgsqlConnection connection,
        NpgsqlTransaction transaction,
        string projectId,
        string userId,
    ...
    )
    {
        ...
        await using (
            var find = DatabaseSql.Command(
                connection,
                transaction,
                "SELECT event_id FROM events WHERE project_id=$1 AND user_id=$2",
                projectId,
                userId
            )
        )
        ...
        await using (
            var findInbox = DatabaseSql.Command(
                connection,
                transaction,
                "SELECT event_id FROM inbox WHERE project_id=$1 AND payload->>'userId'=$2",
                projectId,
                userId
            )
        )
        ...
        if (eventIds.Count > 0)
        {
            ...
            await using (
                var delCh = DatabaseSql.Command(
                    connection,
                    transaction,
                    "DELETE FROM clickhouse_projection WHERE project_id=$1 AND event_id = ANY($2)",
                    projectId,
                    idArray
                )
            )
            ...
            await using (
                var delOutbox = DatabaseSql.Command(
                    connection,
                    transaction,
                    "DELETE FROM broker_outbox WHERE project_id=$1 AND event_id = ANY($2)",
                    projectId,
                    idArray
                )
            )
            ...
            await using (
                var delInbox = DatabaseSql.Command(
                    connection,
                    transaction,
                    "DELETE FROM inbox WHERE project_id=$1 AND event_id = ANY($2)",
                    projectId,
                    idArray
                )
            )
            ...
            await using (
                var delEvents = DatabaseSql.Command(
                    connection,
                    transaction,
                    "DELETE FROM events WHERE project_id=$1 AND user_id=$2",
                    projectId,
                    userId
                )
            )
            ...
            await using (
                var delIdentity = DatabaseSql.Command(
                    connection,
                    transaction,
                    "DELETE FROM event_identity WHERE project_id=$1 AND event_id = ANY($2)",
                    projectId,
                    idArray
                )
            )
            ...
            await using (
                var syncProject = DatabaseSql.Command(
                    connection,
                    transaction,
                    "UPDATE projects SET event_count = (SELECT count(*) FROM event_identity WHERE project_id=$1), stored_bytes = coalesce((SELECT sum(stored_bytes) FROM event_identity WHERE project_id=$1), 0) WHERE id=$1",
                    projectId
                )
            )
    ...
    public async Task<IReadOnlyList<SubjectExportEvent>> ExportSubjectAsync(
        string projectId,
        string userId,
        CancellationToken ct
    )
    {
        ...
        await using var command = DatabaseSql.Command(
            connection,
            null,
            "SELECT event_id, event_type, schema_version, user_id, anonymous_id, session_id, occurred_at, received_at, properties::text FROM events WHERE project_id=$1 AND user_id=$2 ORDER BY occurred_at, event_id",
            projectId,
            userId
        );
        await using var reader = await command.ExecuteReaderAsync(ct);
    ...
    private static async Task<DateTimeOffset> ReceiptAsync(
        NpgsqlConnection connection,
        NpgsqlTransaction transaction,
        string project,
        Guid id,
    ...
    )
    {
        await using var command = DatabaseSql.Command(
            connection,
            transaction,
            "SELECT received_at FROM inbox WHERE project_id=$1 AND event_id=$2",
            project,
            id
        );
    ...
    public async Task<int> RetainAsync(CancellationToken ct)
    {
        ...
        await using (
            var state = DatabaseSql.Command(
                connection,
                transaction,
                "SELECT id FROM storage_state WHERE id = 1 FOR UPDATE"
            )
        )
        ...
        await using (
            var events = DatabaseSql.Command(
                connection,
                transaction,
                """
        ...
        int removed;
        await using (
            var identities = DatabaseSql.Command(
                connection,
                transaction,
                """
                ...
                """,
                now.AddDays(-options.IdentityRetentionDays)
            )
        )
            removed = await identities.ExecuteNonQueryAsync(ct);
        await using (
            var recount = DatabaseSql.Command(
                connection,
                transaction,
                """
