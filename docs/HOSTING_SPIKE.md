...
The milestone 4 image is deployed with `Dashboard__Enabled=true`, and the homepage redirects to `/dashboard/`. Signed-out visitors see the analytics login screen. The `DashboardAccess` migration is applied and the production `demo` account has access to project `a`. The original development shop remains a separate opt-in feature.

On September 17, the HTTPS proxy fix restored `/dashboard-api/auth/token`, but successful logins still alternated between authenticated and unauthorized sessions. Instance-local Data Protection keys were the suspected cause. Migration `20260917143254_SharedDataProtectionKeys` is now applied to Neon; the API stores its key ring in PostgreSQL with the fixed application name `EventTracking`. The rollout requires a fresh login for cookies issued by the previous image. Local verification passed all 49 .NET tests without skips, including login, repeated session reads, authenticated CSRF validation and restart continuity across independent hosts with different content roots, in both durable profiles. See [MILESTONE_4.md](MILESTONE_4.md) for key storage and backup behavior.
