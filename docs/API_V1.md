...
The worker holds transaction-owned `FOR UPDATE SKIP LOCKED` claims, inserts each queryable event idempotently, and marks inbox completion in the same transaction. A connection/process crash rolls back both writes and releases claims. PostgreSQL remains the replay source. Broker delivery, bounded poison-event retries and dead letters are milestone 3.
