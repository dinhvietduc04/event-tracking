...
public sealed class DemoShopTests : IClassFixture<PrototypeFactory>
{
    ...
    public async Task BrowserAndServerEvents_ShareSessionAndHaveDistinctSources()
    {
        ...
        (
            await client.PostAsJsonAsync("/demo/events", new { eventType = "page_view" })
        ...
        var response = await client.PostAsJsonAsync(
            ...
            {
                orderId,
                items = new[] { new { productId = "mug", quantity = 2 } },
                totalMinor = 1,
            }
        );
        ...
        var purchase = Assert.Single(
            events,
            item => item.GetProperty("source").GetString() == "server"
        );
        Assert.Equal("purchase_completed", purchase.GetProperty("eventType").GetString());
        Assert.Equal(
            3600,
            purchase.GetProperty("properties").GetProperty("amountMinor").GetInt32()
        );
    ...
    public async Task ConcurrentOrderRetries_EmitOnePurchase_AndRejectChangedCart()
    {
        ...
        var responses = await Task.WhenAll(
            Enumerable.Range(0, 8).Select(_ => client.PostAsJsonAsync("/demo/orders", order))
        );
        foreach (var response in responses)
        {
            ...
            Assert.Equal(
                order.OrderId,
                (await response.Content.ReadFromJsonAsync<DemoOrder>())!.Id
            );
        }

        // The sentinel is queued after all retries, so observing it proves those messages were processed.
        await client.PostAsJsonAsync("/demo/events", new { eventType = "checkout_started" });
        var events = await ReadUntil(
            client,
            items =>
                items.Any(item => item.GetProperty("eventType").GetString() == "checkout_started")
        );
        Assert.Single(
            events,
            item => item.GetProperty("eventType").GetString() == "purchase_completed"
        );
    ...
    public async Task BrowserCannotEmitPurchase_AndInvalidOrderProducesNoPurchase()
    {
        ...
        Assert.Equal(
            ...
            (await client.GetAsync("/demo/events")).StatusCode
        );
        await client.GetAsync("/demo/bootstrap");
        var forged = await client.PostAsJsonAsync(
            "/demo/events",
            new { eventType = "purchase_completed", source = "server" }
        );
        Assert.Equal(HttpStatusCode.BadRequest, forged.StatusCode);
        DemoCartItem[]?[] invalidCarts =
        [
            null,
            [],
            [new("unknown", 1)],
        ...
        foreach (var items in invalidCarts)
        {
            var invalid = await client.PostAsJsonAsync(
                "/demo/orders",
                new DemoOrderRequest(Guid.NewGuid(), items)
            );
    ...
    public async Task ShopAssetsAreAvailableInDevelopment_AndDemoIsOffOutsideDevelopment()
    {
        ...
        Assert.Equal(
            ...
            (await productionClient.GetAsync("/demo/bootstrap")).StatusCode
        );
        Assert.Equal(
            HttpStatusCode.NotFound,
            (await productionClient.GetAsync("/shop/index.html")).StatusCode
        );
    }

    private static async Task<JsonElement[]> ReadUntil(
        HttpClient client,
