...
public sealed class DashboardTests
{
    ...
    private static Dictionary<string, string?> Settings =>
        new()
        {
            ...
            ["Dashboard:Bootstrap:Username"] = "analyst",
    ...
    private static async Task Login(HttpClient client) =>
        Assert.Equal(
            HttpStatusCode.OK,
            (
                await Post(client, "/auth/login", new { username = "analyst", password = Password })
            ).StatusCode
        );

    private static string Window =>
        $"from={Uri.EscapeDataString(DateTimeOffset.UtcNow.AddDays(-1).ToString("O"))}&to={Uri.EscapeDataString(DateTimeOffset.UtcNow.AddDays(1).ToString("O"))}";

    private static V1EventRequest Event(string type = "page_view") =>
        new(
            Guid.NewGuid(),
            type,
            1,
            ...
            Properties: JsonSerializer.SerializeToElement(new { source = "browser", demo = true })
        );
    ...
    public async Task SharedKeys_ValidateLoginSessionsAndCsrfAcrossInstancesAndRestart()
    {
        foreach (string profile in new[] { "Hosted", "Distributed" })
        {
            ...
            {
                ["Dashboard:Enabled"] = "true",
                ["ReverseProxy:TrustForwardedProto"] = "true",
            };
            ...
            Assert.Equal(
                "EventTracking",
                appA.Services.GetRequiredService<
                    IOptions<DataProtectionOptions>
                >().Value.ApplicationDiscriminator
            );
            Assert.NotEqual(
                appA.Services.GetRequiredService<IWebHostEnvironment>().ContentRootPath,
            ...
            string anonymousToken = (await tokenResponse.Content.ReadFromJsonAsync<JsonElement>())
                .GetProperty("token")
                .GetString()!;
            ...
            for (int i = 0; i < 12; i++)
            {
                ...
                Assert.Equal(
                    "a",
                    Assert
                        .Single(user.GetProperty("projects").EnumerateArray())
                        .GetProperty("id")
                        .GetString()
                );
            }
            using var authenticatedTokenResponse = await Send(
                a,
                HttpMethod.Get,
                "/auth/token",
                cookies
            ...
            string authenticatedToken = (
                await authenticatedTokenResponse.Content.ReadFromJsonAsync<JsonElement>()
            )
                .GetProperty("token")
                .GetString()!;
            Assert.Equal(
                HttpStatusCode.BadRequest,
                (await Send(b, HttpMethod.Post, "/auth/logout", cookies)).StatusCode
            );
            Assert.Equal(
                HttpStatusCode.BadRequest,
                (
            ...
            Assert.Equal(
                ...
                (
                    await Send(b, HttpMethod.Post, "/auth/logout", cookies, authenticatedToken)
                ).StatusCode
            );
            ...
            Assert.Equal(
                HttpStatusCode.OK,
                (await Send(c, HttpMethod.Get, "/session", cookies)).StatusCode
            );
            Assert.Equal(
                HttpStatusCode.NoContent,
                (
                    await Send(c, HttpMethod.Post, "/auth/logout", cookies, authenticatedToken)
                ).StatusCode
            );
            Assert.Equal(
                persistedKeys,
                await db.Scalar("SELECT count(*) FROM data_protection_keys")
            );
        }
    }
    ...
    public async Task LoginRequiresCsrf_ProjectsEnforceMembership_AndLogoutRevokesCookie()
    {
        ...
        Assert.Equal(
            ...
            (await client.GetAsync("/dashboard-api/session")).StatusCode
        );
        Assert.Equal(
            HttpStatusCode.BadRequest,
            (
        ...
        Assert.Equal(
            ...
            (
                await Post(
                    client,
                    "/auth/login",
                    new { username = "analyst", password = "incorrect" }
                )
            ).StatusCode
        );
        await Login(client);
        ...
        Assert.Equal(
            "a",
            Assert
                .Single(session.GetProperty("projects").EnumerateArray())
                .GetProperty("id")
                .GetString()
        );
        ...
        foreach (var endpoint in new[] { "overview", "events" })
        {
            Assert.Equal(
                ...
                (await client.GetAsync($"/dashboard-api/projects/b/{endpoint}?{Window}")).StatusCode
            );
            Assert.Equal(
                HttpStatusCode.OK,
                (
                    await client.GetAsync(
                        $"/dashboard-api/projects/a/{endpoint}?{Window}&projectId=b"
                    )
                ).StatusCode
            );
        }
        Assert.Equal(
            HttpStatusCode.Forbidden,
            (
                await Post(client, "/projects/b/demo/events", new { events = new[] { Event() } })
            ).StatusCode
        );
        ...
        Assert.Equal(
            HttpStatusCode.BadRequest,
            (
                await client.PostAsJsonAsync("/dashboard-api/projects", new { name = "Forged" })
            ).StatusCode
        );
        var created = await (
            await Post(client, "/projects", new { name = "My new project" })
        ).Content.ReadFromJsonAsync<DashboardProject>();
        Assert.NotNull(created);
        Assert.True(created.CanDemo);
        Assert.Equal(
            HttpStatusCode.OK,
            (
                await client.GetAsync($"/dashboard-api/projects/{created.Id}/overview?{Window}")
            ).StatusCode
        );
        Assert.Equal(
            HttpStatusCode.NoContent,
            (await Post(client, "/auth/logout", new { })).StatusCode
        );
        Assert.Equal(
            HttpStatusCode.Unauthorized,
            (await client.GetAsync("/dashboard-api/session")).StatusCode
        ...
        Assert.Equal(
            1,
            await db.Scalar(
                "SELECT count(*) FROM dashboard_users WHERE password_hash NOT LIKE '%synthetic%'"
            )
        );
    }
    ...
    public async Task PutRequiresCsrf_AndSucceedsWithToken()
    {
        ...
        using (
            var forged = new HttpRequestMessage(
                HttpMethod.Put,
                $"/dashboard-api/projects/a/views/{viewId:D}"
            )
            {
    ...
    public async Task DemoActionsReachDashboard_WithStablePurchaseRetriesAndScopedFilters()
    {
        foreach (string profile in new[] { "Hosted", "Distributed" })
        {
            ...
            (
                await Post(client, "/projects/a/demo/events", new { events })
            ).EnsureSuccessStatusCode();
            var retry = await (
                await Post(client, "/projects/a/demo/purchase", purchase)
            ...
            Assert.Equal(
                ...
                (
                    await Post(client, "/projects/a/demo/purchase", purchase with { Quantity = 3 })
                ).StatusCode
            );
            Assert.Equal(
                HttpStatusCode.BadRequest,
            ...
            Assert.Equal(
                ...
                (
                    await Post(
                        client,
                        "/projects/a/demo/events",
                        new { events = new[] { Event("purchase") } }
                    )
                ).StatusCode
            );
            Assert.Equal(
            ...
            {
                var pending = await client.GetFromJsonAsync<JsonElement>(
                    $"/dashboard-api/projects/a/overview?{window}"
                );
            ...
            Assert.Equal(
                1,
                overview.GetProperty("users").GetProperty("identifiedUsers").GetInt32()
            );
            Assert.Equal(0, overview.GetProperty("status").GetProperty("pending").GetInt32());
            if (profile == "Distributed")
                Assert.True(
                    overview.GetProperty("status").GetProperty("processingP95Seconds").GetDouble()
                        >= 0
                );
            var first = (
                await client.GetFromJsonAsync<TimelinePage>(
                    $"/dashboard-api/projects/a/events?{window}&limit=1"
                )
            )!;
            ...
            var second = (
                await client.GetFromJsonAsync<TimelinePage>(
                    $"/dashboard-api/projects/a/events?{window}&limit=1&cursor={Uri.EscapeDataString(first.NextCursor)}"
                )
            )!;
            Assert.NotEqual(first.Events[0].EventId, second.Events[0].EventId);
            var filtered = (
                await client.GetFromJsonAsync<TimelinePage>(
                    $"/dashboard-api/projects/a/events?{window}&userId=demo-user-001&propertyName=source&propertyValue=%22server%22"
                )
            )!;
            ...
            Assert.Equal(
                ...
                (
                    await client.GetAsync(
                        $"/dashboard-api/projects/a/events?{window}&eventType=login&cursor={Uri.EscapeDataString(first.NextCursor)}"
                    )
                ).StatusCode
            );
        }
    ...
    public async Task MembershipRemovalAndAccountDisablingTakeEffectOnExistingSessions()
    {
        ...
        Assert.Equal(
            HttpStatusCode.Forbidden,
            (
                await Post(client, "/projects/a/demo/events", new { events = new[] { Event() } })
            ).StatusCode
        );
        Assert.Equal(
            HttpStatusCode.Forbidden,
            (
                await Post(
                    client,
                    "/projects/a/demo/purchase",
                    new DemoPurchase(Guid.NewGuid(), DateTimeOffset.UtcNow, "s", "mug", 1)
                )
            ).StatusCode
        );
        Assert.Equal(
            HttpStatusCode.OK,
            (await client.GetAsync($"/dashboard-api/projects/a/events?{Window}")).StatusCode
        );
    ...
    public async Task NonDevelopmentDashboardIsOptIn_AndBootstrapDoesNotResetStoredAccounts()
    {
        ...
        await db.Execute(
            "UPDATE dashboard_users SET disabled=true; DELETE FROM project_memberships"
        );
        ...
        Assert.Equal(
            ...
            (await prodClient.GetAsync("/dashboard-api/auth/token")).StatusCode
        );
        using var enabled = db.App(
            "Hosted",
            seed: false,
            settings: new() { ["Dashboard:Enabled"] = "true" }
        );
        ...
        Assert.Equal(
            ...
            (await enabledClient.GetAsync("/dashboard/")).StatusCode
        );
        Assert.Equal(
            HttpStatusCode.Unauthorized,
            (await enabledClient.GetAsync("/dashboard-api/session")).StatusCode
        );
        Assert.Equal(
            HttpStatusCode.NotFound,
            (await enabledClient.GetAsync("/shop/index.html")).StatusCode
        );
    }
            ...
            Assert.Equal(
                ...
                (
                    await Post(
                        client,
                        "/auth/login",
                        new { username = "missing", password = "incorrect" }
                    )
                ).StatusCode
            );
        Assert.Equal(
