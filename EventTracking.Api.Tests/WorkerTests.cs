...
public sealed class WorkerTests
{
    ...
    public async Task IndependentWorkerCrashRollsBack_AndTwoProcessesRecoverWithoutApi()
    {
        ...
        using (var client = api.CreateClient().WithKey(TestProjects.IngestA))
        {
            Assert.DoesNotContain(
                api.Services.GetServices<IHostedService>(),
                service => service is PostgresWorker
            );
            Assert.Equal(
        ...
        await using (var hold = new NpgsqlCommand("SELECT pg_advisory_lock(4343)", blocker))
        ...
        using (var crashed = new WorkerProcess(db.ConnectionString, "crashed-worker"))
        {
            await Until(
                async () =>
                    await db.Scalar(
                        "SELECT count(*) FROM pg_stat_activity WHERE application_name='crashed-worker' AND wait_event='advisory'"
                    ) == 1,
                crashed
            );
            crashed.Kill();
        }
        await using (var release = new NpgsqlCommand("SELECT pg_advisory_unlock(4343)", blocker))
            await release.ExecuteNonQueryAsync();
        // Prove the connection/claim is gone, then inspect the durable unprocessed state.
        await Until(async () =>
            await db.Scalar(
                "SELECT count(*) FROM pg_stat_activity WHERE application_name='crashed-worker'"
            ) == 0
        );
        Assert.Equal(0, await db.Scalar("SELECT count(*) FROM events"));
        ...
        await Until(
            async () =>
                await db.Scalar(
                    "SELECT count(*) FROM pg_stat_activity WHERE application_name IN ('first-worker','second-worker') AND wait_event='advisory'"
                ) == 2,
            first,
            second
        );
        ...
        await Until(
            async () =>
                await db.Scalar("SELECT count(*) FROM inbox WHERE processed_at IS NULL") == 0,
            first,
            second
        );
        ...
        Assert.Equal(
            0,
            await db.Scalar(
                "SELECT count(*) FROM event_identity d LEFT JOIN events e USING(project_id,event_id) WHERE e.event_id IS NULL"
            )
        );
    ...
    public async Task WorkerRefusesHostedDatabaseAndDoesNotMigrateEmptyDatabase()
    {
        ...
        Assert.Equal(
            1,
            await db.Scalar("SELECT count(*) FROM storage_state WHERE profile='Hosted'")
        );
    ...
    public async Task WorkerRejectsProjectionAfterDatabaseTransitionsToHosted()
    {
        ...
        using var hosted = db.App(
            "Hosted",
            settings: new() { ["Storage:AllowProfileTransition"] = "true" }
        );
        using var newClient = hosted.CreateClient();
        await Assert.ThrowsAsync<StorageProfileException>(() =>
            distributed.Services.GetRequiredService<PostgresStore>().ProcessBatchAsync(default)
        );
    }
    ...
    public async Task BrokerOutbox_PublishesAndCompletesDelivery_Idempotently()
    {
        ...
        Assert.Equal(
            HttpStatusCode.Accepted,
            (
                await client.PostAsJsonAsync("/v1/events/batch", new { events = new[] { payload } })
            ).StatusCode
        );
        Assert.Equal(
            1,
            await db.Scalar(
                "SELECT count(*) FROM broker_outbox WHERE completed_at IS NULL AND dead_lettered_at IS NULL"
            )
        );

        // Publish via an injectable transport (no live broker required).
        List<BrokerMessage> published = [];
        int count = await store.PublishBrokerOutboxAsync(
            message =>
            {
        ...
        Assert.Equal(
            1,
            await db.Scalar("SELECT count(*) FROM broker_outbox WHERE completed_at IS NOT NULL")
        );
    }
    ...
    public async Task BrokerOutage_LeavesOutboxPending_AndRecoversOnRetry()
    {
        ...
        await Assert.ThrowsAsync<IOException>(() =>
            store.PublishBrokerOutboxAsync(
                _ => throw new IOException("broker unreachable"),
                default
            )
        );
        Assert.Equal(
        ...
        Assert.Equal(
            1,
            await store.PublishBrokerOutboxAsync(
                message =>
                {
    ...
    private sealed class WorkerProcess : IDisposable
    {
        ...
        public WorkerProcess(string connection, string name)
        {
            ...
            start.ArgumentList.Add(
                Path.Combine(AppContext.BaseDirectory, "worker-process", "EventTracking.Worker.dll")
            );
            start.Environment["DOTNET_ENVIRONMENT"] = "Testing";
            start.Environment["ConnectionStrings__Tracking"] = new NpgsqlConnectionStringBuilder(
                connection
            )
            {
            ...
            foreach (
                var argument in new[]
                {
                    ...
                    "--Storage:WorkerBatchSize=1",
                    "--Storage:PollIntervalMilliseconds=50",
        ...
        public void AssertRunning() =>
            Assert.False(
                process.HasExited,
                "Independent worker exited before projection completed: "
