...
public sealed class SubjectDataTests
{
    ...
    public async Task ExportAndDeleteSubject_RemovesAllSubjectTraces()
    {
        ...
        var events = new[]
        {
            new { eventId = Guid.NewGuid(), eventType = "signup", schemaVersion = 1, userId = targetUser, occurredAt = DateTimeOffset.UtcNow },
            new { eventId = Guid.NewGuid(), eventType = "login", schemaVersion = 1, userId = targetUser, occurredAt = DateTimeOffset.UtcNow },
            new { eventId = Guid.NewGuid(), eventType = "login", schemaVersion = 1, userId = otherUser, occurredAt = DateTimeOffset.UtcNow }
        };
        ...
        // Export subject data
        var exportRes = await admin.GetAsync($"/dashboard-api/projects/a/admin/subjects/{targetUser}/export");
        ...
        long remainingTargetInbox = await db.Scalar($"SELECT count(*) FROM inbox WHERE project_id='a' AND payload->>'userId'='{targetUser}'");
        Assert.Equal(0, remainingTargetInbox);

        long remainingOtherInbox = await db.Scalar($"SELECT count(*) FROM inbox WHERE project_id='a' AND payload->>'userId'='{otherUser}'");
    ...
    public async Task DeleteSubject_SurvivesRetention_AndReplayDoesNotRecreate()
    {
        ...
        {
            events = new[] { new { eventId, eventType = "signup", schemaVersion = 1, userId = targetUser, occurredAt = DateTimeOffset.UtcNow } }
        });
        Assert.Equal(HttpStatusCode.Accepted, ingestRes.StatusCode);
        ...
        Assert.Equal(0, await db.Scalar($"SELECT count(*) FROM events WHERE project_id='a' AND user_id='{targetUser}'"));
        Assert.Equal(0, await db.Scalar($"SELECT count(*) FROM inbox WHERE project_id='a' AND payload->>'userId'='{targetUser}'"));
        Assert.Equal(0, await db.Scalar($"SELECT count(*) FROM broker_outbox WHERE project_id='a' AND event_id='{eventId}'"));
