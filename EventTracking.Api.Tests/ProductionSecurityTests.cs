...
public sealed class ProductionSecurityTests
{
    ...
    private static Dictionary<string, string?> Safe =>
        new()
        {
            ["AllowedHosts"] = "analytics.example.test",
            ["ConnectionStrings:Tracking"] =
                "Host=database.example.test;Database=tracking;Username=runtime;SSL Mode=VerifyFull;GSS Encryption Mode=Disable",
        };
    ...
    public void ProductionRejectsUnsafeRuntimeConfiguration(string setting, string value)
    {
        ...
        var error = Assert.Throws<InvalidOperationException>(() =>
            ProductionSecurity.Validate(
                config,
                config.GetSection("Storage").Get<StorageOptions>() ?? new(),
                strict: true,
                operatorMode: false,
                api: true
            )
    ...
    {
        var values = Safe;
        values["ConnectionStrings:Tracking"] =
            $"Host=database.example.test;Username=runtime;SSL Mode={ssl};GSS Encryption Mode=Disable";
        Assert.Throws<InvalidOperationException>(() =>
            ProductionSecurity.Connection(Config(values), strict: true, operatorMode: false)
        );
        Assert.NotEmpty(
            ProductionSecurity.Connection(Config(Safe), strict: true, operatorMode: false)
        );
    }
    ...
    {
        Assert.True(ProductionSecurity.Required("Production"));
        ...
        Assert.Throws<InvalidOperationException>(() =>
            ProductionSecurity.Connection(Config(values), true, true)
        );
        ...
        Assert.Throws<InvalidOperationException>(() =>
            ProtectionCertificates.Load(Config(Safe), required: true)
        );
        values["DataProtection:Certificate:Base64"] = "invalid-private-certificate-secret";
        var error = Assert.Throws<InvalidOperationException>(() =>
            ProtectionCertificates.Load(Config(values), true)
        );
    ...
    public async Task RuntimeRolesAllowTheirWork_ButRejectMigrationRecoveryAndAuditTampering()
    {
        foreach (string profile in new[] { "Hosted", "Distributed" })
        {
            ...
            using var seed = db.App(
                profile,
                settings: new()
                {
                    ["Dashboard:Bootstrap:Username"] = "security-test",
                    ["Dashboard:Bootstrap:Password"] = "synthetic-security-password",
            ...
            await using (
                var disguisedOwner = NpgsqlDataSource.Create(
                    new NpgsqlConnectionStringBuilder(db.ConnectionString)
                    {
                        Options = "-c role=" + api.Name,
                    }.ConnectionString
                )
            )
                await Assert.ThrowsAsync<InvalidOperationException>(() =>
                    RuntimeDatabaseAccess.Verify(disguisedOwner, "Api")
                );
            using var app = db.App(
                ...
                settings: new()
                {
                    ["ConnectionStrings:Tracking"] = api.ConnectionString,
            ...
            using var client = app.CreateClient(
                ...
                .WithKey(TestProjects.BothA);
            var payload = new V1EventRequest(
                Guid.NewGuid(),
                "restricted_role",
                1,
            ...
            using var login = new HttpRequestMessage(HttpMethod.Post, "/dashboard-api/auth/login")
            {
                Content = JsonContent.Create(
                    new { username = "security-test", password = "synthetic-security-password" }
                ),
            };
            ...
            await using (
                var query = DatabaseSql.Command(
                    connection,
                    null,
                    "SELECT id FROM dashboard_users WHERE username='security-test'"
                )
            )
                user = (Guid)(await query.ExecuteScalarAsync())!;
            ...
            await Assert.ThrowsAsync<InvalidOperationException>(() =>
                RuntimeDatabaseAccess.Verify(api.Source, "Api")
            );
    ...
    public async Task SharedLoginBudgetSurvivesInstanceChangesAndUsesBoundedStorage()
    {
        ...
        for (int attempt = 0; attempt < 10; attempt++)
            Assert.True(await (attempt % 2 == 0 ? a : b).Acquire("Case-Normalized", default));
        Assert.False(await b.Acquire("  case-normalized  ", default));
        using var restarted = db.App("Hosted", seed: false);
        Assert.False(
            await restarted
                .Services.GetRequiredService<SharedLoginLimiter>()
                .Acquire("CASE-NORMALIZED", default)
        );
        await db.Execute(
            "UPDATE login_rate_limits SET window_start=window_start-interval '2 minutes'"
        );
        ...
        Assert.Equal(
            0,
            await db.Scalar(
                "SELECT count(*) FROM login_rate_limits WHERE bucket LIKE '%normalized%'"
            )
        );
    ...
    public async Task ProtectionKeysCanBeEncrypted_RotatedAndRecoveredWithoutChangingCookies()
    {
        ...
        using (var plaintext = db.App("Hosted"))
        {
            original = plaintext
                ...
                .CreateProtector("recovery-test")
                .Protect("synthetic-session");
            await Assert.ThrowsAsync<InvalidOperationException>(() =>
                ProtectionCertificates.VerifyStoredKeys(plaintext.Services)
            );
        }
        ...
        Assert.Equal(
            0,
            await db.Scalar("SELECT count(*) FROM data_protection_keys WHERE xml LIKE '%<value>%'")
        );
        ...
        rotation["DataProtection:PreviousCertificates:0:Base64"] = firstSettings[
            "DataProtection:Certificate:Base64"
        ];
        ...
        using (
            var recovery = db.App(
                "Hosted",
                seed: false,
                settings: CertificateSettings(secondCertificate)
            )
        )
        {
        ...
        await Assert.ThrowsAnyAsync<CryptographicException>(() =>
            ProtectionCertificates.VerifyStoredKeys(wrong.Services)
        );
        Assert.Equal(
            2,
            await db.Scalar(
                "SELECT count(*) FROM audit_records WHERE action='protection.keys-wrapped'"
            )
        );
    }
    ...
    private static Dictionary<string, string?> CertificateSettings(X509Certificate2 certificate) =>
        new()
        {
            ["DataProtection:Certificate:Base64"] = Convert.ToBase64String(
                certificate.Export(X509ContentType.Pfx, "synthetic-pfx-password")
            ),
    ...
    private sealed class RuntimeRole(
    ...
    ) : IAsyncDisposable
    {
        ...
        public static async Task<RuntimeRole> Create(
            PostgresTestDatabase db,
            NpgsqlDataSource owner,
            string kind
        )
        {
            string name = "m5_role_" + Guid.NewGuid().ToString("N"),
            ...
            string connection = new NpgsqlConnectionStringBuilder(db.ConnectionString)
            {
                Username = name,
                Password = password,
            }.ConnectionString;
            ...
            try
            {
                await RuntimeDatabaseAccess.Grant(owner, name, kind);
                return role;
            }
            catch
            {
