...
public sealed class BrokerTests
{
    ...
    {
        var metrics = new OutboxMetrics(Pending: 10, OldestPendingSeconds: 5.4, Completed: 100, DeadLettered: 2, Retrying: 1);
    ...
    public async Task OutboxQuota_RejectsEventsWhenPendingExceedsLimit()
    {
        ...
        var events1 = new[]
        {
            new { eventId = Guid.NewGuid(), eventType = "test_event", schemaVersion = 1, occurredAt = DateTimeOffset.UtcNow },
            new { eventId = Guid.NewGuid(), eventType = "test_event", schemaVersion = 1, occurredAt = DateTimeOffset.UtcNow }
        };
    ...
    private static async Task<HttpResponseMessage> Post(HttpClient client, string path, object? value = null)
    {
        var token = await client.GetFromJsonAsync<JsonElement>("/dashboard-api/auth/token");
        using var message = new HttpRequestMessage(HttpMethod.Post, "/dashboard-api" + path) { Content = value is null ? null : JsonContent.Create(value) };
    ...
    private static async Task Login(HttpClient client) =>
        Assert.Equal(HttpStatusCode.OK, (await Post(client, "/auth/login", new { username = "admin", password = Password })).StatusCode);
    private static Task Operate(WebApplicationFactory<Program> app, string action) =>
        DashboardOperator.Execute(app.Services.GetRequiredService<NpgsqlDataSource>(), new ConfigurationBuilder().AddInMemoryCollection(new Dictionary<string, string?>
        {
            ["Administration:Action"] = action, ["Administration:Username"] = "admin",
    ...
    public async Task DeadLetter_InspectAndReplayEndpoints_WorkCorrectly()
    {
        ...
        {
            var ingestRes = await ingest.PostAsJsonAsync("/v1/events/batch", new
            {
                events = new[] { new { eventId, eventType = "dlq_test", schemaVersion = 1, occurredAt = DateTimeOffset.UtcNow } }
            });
        ...
        long remaining = await db.Scalar($"SELECT count(*) FROM broker_outbox WHERE project_id='a' AND dead_lettered_at IS NOT NULL");
    ...
    {
        var eventId = Guid.NewGuid();
        var res = await ingest.PostAsJsonAsync("/v1/events/batch", new
        {
            events = new[] { new { eventId, eventType = "broker_test", schemaVersion = 1, occurredAt = DateTimeOffset.UtcNow } }
        });
    ...
    public async Task PermanentFailure_DeadLettersImmediately()
    {
        ...
        await store.RecordBrokerFailureAsync(new BrokerMessage("a", eventId, "{}"),
            new System.Text.Json.JsonException("poison"), permanent: true, default);
        Assert.Equal(1, await db.Scalar(
            $"SELECT count(*) FROM broker_outbox WHERE project_id='a' AND event_id='{eventId}' AND dead_lettered_at IS NOT NULL"));
    }
    ...
    {
        await using var db = await PostgresTestDatabase.Create();
        var settings = new Dictionary<string, string?> { ["RabbitMq:Enabled"] = "true", ["RabbitMq:MaxAttempts"] = "3" };
        using var app = db.App("Distributed", seed: true, settings);
        var store = app.Services.GetRequiredService<PostgresStore>();
        using var ingest = app.CreateClient().WithKey(TestProjects.IngestA);
        var eventId = await IngestOne(ingest);
        ...
        await store.RecordBrokerFailureAsync(message, new TimeoutException("db busy"), permanent: false, default);
        Assert.Equal(0, await db.Scalar(
            $"SELECT count(*) FROM broker_outbox WHERE project_id='a' AND event_id='{eventId}' AND dead_lettered_at IS NOT NULL"));
        Assert.Equal(1, await db.Scalar(
            $"SELECT count(*) FROM broker_outbox WHERE project_id='a' AND event_id='{eventId}' AND next_attempt_at > now() AND attempts = 1"));
        await store.RecordBrokerFailureAsync(message, new Npgsql.NpgsqlException("connection reset"), permanent: false, default);
        Assert.Equal(1, await db.Scalar(
        ...
        await store.RecordBrokerFailureAsync(message, new TimeoutException("still down"), permanent: false, default);
        Assert.Equal(1, await db.Scalar(
            $"SELECT count(*) FROM broker_outbox WHERE project_id='a' AND event_id='{eventId}' AND dead_lettered_at IS NOT NULL"));
    }
    ...
    public async Task Replay_PreservesOriginalEventIdsAndCounts()
    {
        ...
        await db.Execute("""
            ...
            """);
        Assert.Equal(2, await db.Scalar(
        ...
        Assert.Equal(1, await db.Scalar($"SELECT count(*) FROM broker_outbox WHERE project_id='a' AND event_id='{second}' AND attempts = 0"));
    }
}
