...
public sealed class AdministrationTests
{
    ...
    private static async Task Login(
        ...
        Assert.Equal(
            HttpStatusCode.OK,
            (await Post(client, "/auth/login", new { username, password })).StatusCode
        );

    private static IConfiguration Config(Dictionary<string, string?> values) =>
        new ConfigurationBuilder().AddInMemoryCollection(values).Build();

    private static Task Operate(
    ...
    private static Task<bool> Provision(
        WebApplicationFactory<Program> app,
        string username,
        string projects = "a"
    ) =>
        app
            .Services.GetRequiredService<DashboardAccounts>()
            .Provision(
                Config(
                    new()
                    {
                        ["Dashboard:Bootstrap:Username"] = username,
                        ["Dashboard:Bootstrap:Password"] = Password,
    ...
    public async Task RolesRequireExplicitGrant_AndNewProjectsMakeTheirCreatorAdmin()
    {
        ...
        foreach (string route in new[] { "members", "keys", "audit" })
            Assert.Equal(
                ...
                (await admin.GetAsync($"/dashboard-api/projects/a/admin/{route}")).StatusCode
            );
        ...
        Assert.Equal(
            ...
            (
                await admin.PostAsJsonAsync(
                    "/dashboard-api/projects/a/admin/keys",
                    new { permissions = new[] { "read" } }
                )
            ).StatusCode
        );
        ...
        Assert.Equal(
            ...
            (
                await Post(
                    admin,
                    "/projects/a/admin/members",
                    new { username = "viewer", role = "viewer" }
                )
            ).StatusCode
        );
        using var viewer = Client(app);
        await Login(viewer, "viewer");
        Assert.Equal(
            HttpStatusCode.OK,
            (await viewer.GetAsync("/dashboard-api/session")).StatusCode
        );
        Assert.Equal(
            HttpStatusCode.Forbidden,
            (await Post(viewer, "/projects/a/demo/purchase", new { })).StatusCode
        );
        Assert.Equal(
            HttpStatusCode.Forbidden,
            (
                await Post(
                    viewer,
                    "/projects/a/admin/members",
                    new { username = "viewer", role = "admin" }
                )
            ).StatusCode
        );
        Assert.Equal(
            HttpStatusCode.Forbidden,
            (
                await Post(viewer, "/projects/a/admin/keys", new { permissions = new[] { "read" } })
            ).StatusCode
        );
        Assert.Equal(
            HttpStatusCode.NoContent,
            (
                await Post(
                    admin,
                    "/projects/a/admin/members",
                    new { username = "viewer", role = "contributor" }
                )
            ).StatusCode
        );
        ...
        Assert.Equal(
            "contributor",
            session.GetProperty("projects")[0].GetProperty("role").GetString()
        );
        Assert.Equal(
            HttpStatusCode.Forbidden,
            (await viewer.GetAsync("/dashboard-api/projects/a/admin/audit")).StatusCode
        );
        var created = await (
            await Post(viewer, "/projects", new { name = "Owned project" })
        ).Content.ReadFromJsonAsync<DashboardProject>();
        ...
        Assert.Equal(
            HttpStatusCode.OK,
            (
                await viewer.GetAsync($"/dashboard-api/projects/{created.Id}/admin/members")
            ).StatusCode
        );
        Assert.Equal(
            HttpStatusCode.Forbidden,
            (await admin.GetAsync($"/dashboard-api/projects/{created.Id}/admin/members")).StatusCode
        );
        Assert.Equal(
            ...
            (
                await Post(
                    admin,
                    "/projects/a/admin/members",
                    new { username = "admin", role = "remove" }
                )
            ).StatusCode
        );
        Assert.Equal(
            ...
            (
                await Post(
                    admin,
                    "/projects/a/admin/members",
                    new { username = "viewer", role = "remove" }
                )
            ).StatusCode
        );
        Assert.Equal(
            HttpStatusCode.Forbidden,
            (await viewer.GetAsync("/dashboard-api/projects/a/admin/members")).StatusCode
        );
    }
    ...
    public async Task KeysAreScoped_RotateAtomically_AndNeverAppearInAuditOrLists()
    {
        ...
        await Login(admin);
        await Operate(app, "grant-admin");
        var issuedResponse = await Post(
            admin,
            "/projects/a/admin/keys",
            new { permissions = new[] { "read" } }
        );
        ...
        string key = issued.GetProperty("key").GetString()!,
            hash = issued.GetProperty("keyHash").GetString()!;
        ...
        Assert.Equal(
            HttpStatusCode.Forbidden,
            (await reader.PostAsJsonAsync("/v1/events", new { })).StatusCode
        );
        Assert.Equal(
            HttpStatusCode.Unauthorized,
            (await reader.GetAsync("/dashboard-api/projects/a/admin/keys")).StatusCode
        );
        string foreignHash = Convert.ToHexString(
            SHA256.HashData(Encoding.UTF8.GetBytes(TestProjects.ReadB))
        );
        Assert.Equal(
            HttpStatusCode.NotFound,
            (
                await Post(admin, "/projects/a/admin/keys/revoke", new { keyHash = foreignHash })
            ).StatusCode
        );
        Assert.Equal(
            HttpStatusCode.NotFound,
            (
                await Post(admin, "/projects/a/admin/keys/rotate", new { keyHash = foreignHash })
            ).StatusCode
        );
        Assert.Equal(
            HttpStatusCode.Forbidden,
            (
                await Post(admin, "/projects/b/admin/keys", new { permissions = new[] { "read" } })
            ).StatusCode
        );
        var rotated = await (
            await Post(admin, "/projects/a/admin/keys/rotate", new { keyHash = hash })
        ).Content.ReadFromJsonAsync<JsonElement>();
        string nextKey = rotated.GetProperty("key").GetString()!,
            nextHash = rotated.GetProperty("keyHash").GetString()!;
        Assert.NotEqual(key, nextKey);
        Assert.Equal(
            HttpStatusCode.Unauthorized,
            (await reader.GetAsync("/v1/analytics/events")).StatusCode
        );
        reader.WithKey(nextKey);
        Assert.Equal(HttpStatusCode.OK, (await reader.GetAsync("/v1/analytics/events")).StatusCode);
        Assert.Equal(
            HttpStatusCode.NotFound,
            (await Post(admin, "/projects/a/admin/keys/rotate", new { keyHash = hash })).StatusCode
        );
        foreach (string endpoint in new[] { "keys", "audit" })
        {
            ...
            Assert.DoesNotContain(nextKey, body);
            Assert.DoesNotContain(Password, body);
        }
        Assert.Equal(
            1,
            await db.Scalar("SELECT count(*) FROM audit_records WHERE action='key.rotated'")
        );
        Assert.Equal(
            HttpStatusCode.NoContent,
            (
                await Post(admin, "/projects/a/admin/keys/revoke", new { keyHash = nextHash })
            ).StatusCode
        );
        Assert.Equal(
            HttpStatusCode.Unauthorized,
    ...
    public async Task PasswordResetAndDisableInvalidateSessionsAcrossInstances_WithoutBootstrapResurrection()
    {
        ...
        Assert.Equal(
            HttpStatusCode.Unauthorized,
            (await stale.GetAsync("/dashboard-api/session")).StatusCode
        );
        using var fresh = Client(app);
        Assert.Equal(
            HttpStatusCode.Unauthorized,
            (
                await Post(fresh, "/auth/login", new { username = "admin", password = Password })
            ).StatusCode
        );
        ...
        Assert.Equal(
            ...
            (
                await Post(
                    disabled,
                    "/auth/login",
                    new { username = "admin", password = replacement }
                )
            ).StatusCode
        );
        ...
        Assert.Equal(
            HttpStatusCode.Unauthorized,
            (await fresh.GetAsync("/dashboard-api/session")).StatusCode
        );
        await Login(fresh, password: replacement);
        Assert.Equal(
            3,
            await db.Scalar("SELECT session_version FROM dashboard_users WHERE username='admin'")
        );
        Assert.Equal(
            0,
            await db.Scalar(
                "SELECT count(*) FROM audit_records WHERE details::text LIKE '%password%'"
            )
        );
        Assert.Equal(
            3,
            await db.Scalar(
                "SELECT count(*) FROM audit_records WHERE actor='operator:test-operator'"
            )
        );
    }
    ...
    public async Task AuditFailureRollsBackKeyRotation_AndAccountRecovery()
    {
        ...
        await Operate(app, "grant-admin");
        var issued = await (
            await Post(admin, "/projects/a/admin/keys", new { permissions = new[] { "read" } })
        ).Content.ReadFromJsonAsync<JsonElement>();
        long keyCount = await db.Scalar("SELECT count(*) FROM credentials");
        await db.Execute(
            "CREATE FUNCTION reject_test_audit() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN RAISE EXCEPTION 'synthetic audit failure'; END $$; CREATE TRIGGER reject_test_audit BEFORE INSERT ON audit_records FOR EACH ROW EXECUTE FUNCTION reject_test_audit()"
        );
        Assert.Equal(
            ...
            (
                await Post(
                    admin,
                    "/projects/a/admin/keys/rotate",
                    new { keyHash = issued.GetProperty("keyHash").GetString() }
                )
            ).StatusCode
        );
        ...
        await Assert.ThrowsAsync<PostgresException>(() =>
            Operate(app, "reset-password", password: "synthetic-password-should-rollback")
        );
        Assert.Equal(
            0,
            await db.Scalar("SELECT session_version FROM dashboard_users WHERE username='admin'")
        ...
        Assert.Equal(
            HttpStatusCode.OK,
            (await admin.GetAsync("/dashboard-api/session")).StatusCode
        );
    }
    ...
    public async Task ConcurrentAdministratorsCannotRemoveEachOtherAndLeaveNoAdmin()
    {
        ...
        await Login(first);
        await Operate(app, "grant-admin");
        Assert.True(await Provision(app, "second-admin"));
        await Operate(app, "grant-admin", "second-admin");
        using var second = Client(app);
        await Login(second, "second-admin");
        var responses = await Task.WhenAll(
            Post(
                first,
                "/projects/a/admin/members",
                new { username = "second-admin", role = "remove" }
            ),
        ...
        Assert.Equal(
            1,
            await db.Scalar(
                "SELECT count(*) FROM project_memberships WHERE project_id='a' AND can_manage"
            )
        );
    }
    ...
    public async Task AdministrativeLimitsAreEnforcedUnderConcurrentKeyIssuance()
    {
        ...
        await db.Execute(
            "INSERT INTO credentials(key_hash,project_id,permissions,revoked) SELECT lpad(to_hex(n),64,'0'),'a',ARRAY['read'],false FROM generate_series(1,16) n"
        );
        long auditCount = await db.Scalar("SELECT count(*) FROM audit_records");
        Assert.Equal(
            HttpStatusCode.BadRequest,
            (
                await Post(first, "/projects/a/admin/keys", new { permissions = new[] { "admin" } })
            ).StatusCode
        );
        ...
        Assert.Equal(
            20,
            await db.Scalar("SELECT count(*) FROM credentials WHERE project_id='a' AND NOT revoked")
        );
        // Rotation replaces one active credential and remains possible at the limit.
        string existingHash = Convert.ToHexString(
            SHA256.HashData(Encoding.UTF8.GetBytes(TestProjects.ReadA))
        );
        Assert.Equal(
            HttpStatusCode.OK,
            (
                await Post(first, "/projects/a/admin/keys/rotate", new { keyHash = existingHash })
            ).StatusCode
        );
        Assert.Equal(
            20,
        ...
        Assert.True(
            await Provision(
                app,
                "full-account",
                string.Join(',', Enumerable.Range(1, 20).Select(i => "quota-" + i))
            )
        );
        Assert.Equal(
            HttpStatusCode.Conflict,
            (
                await Post(
                    first,
                    "/projects/a/admin/members",
                    new { username = "full-account", role = "viewer" }
                )
            ).StatusCode
        );
        Assert.Equal(
            20,
            await db.Scalar(
                "SELECT count(*) FROM project_memberships m JOIN dashboard_users u ON u.id=m.user_id WHERE u.username='full-account'"
            )
        );
        ...
        Assert.Equal(
            ...
            (
                await Post(
                    first,
                    "/projects/a/admin/members",
                    new { username = "full-account", role = "admin" }
                )
            ).StatusCode
        );
    }
    ...
    public async Task MigrationPreservesExistingMembershipsWithoutGrantingAdministrativeAccess()
    {
        ...
        Assert.Equal(
            1,
            await db.Scalar(
                "SELECT count(*) FROM project_memberships WHERE can_demo AND NOT can_manage"
            )
        );
        Assert.Equal(
            0,
            await db.Scalar("SELECT session_version FROM dashboard_users WHERE username='legacy'")
        );
    }
}
