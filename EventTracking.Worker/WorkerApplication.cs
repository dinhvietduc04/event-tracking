...
public static class WorkerApplication
{
    public static IHost Build(string[] args, Action<HostApplicationBuilder>? configure = null)
    {
        ...
        if (rabbitMq.Enabled) builder.Services.AddHostedService<RabbitMqWorker>();
        else builder.Services.AddHostedService<PostgresWorker>();
        ...
        builder.Services.Configure<HostOptions>(options => options.ShutdownTimeout = TimeSpan.FromSeconds(storage.WorkerShutdownTimeoutSeconds));
